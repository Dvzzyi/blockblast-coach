...
class OverlayService : Service() {
    ...
    private fun showFab() {
        ...
        fab.setOnTouchListener { v, ev ->
            when (ev.action) {
                MotionEvent.ACTION_DOWN -> { dx = params.x - ev.rawX; dy = params.y - ev.rawY; false }
                MotionEvent.ACTION_MOVE -> {
                    params.x = (ev.rawX + dx).toInt()
                    params.y = (ev.rawY + dy).toInt()
    ...
    private fun bitmapToBase64(bmp: Bitmap): String {
        // Reduz pra 720x1600 max
        val scaled = Bitmap.createScaledBitmap(bmp, 720, 1600, true)
...
{
  "pieces": ["descrição curta peça 1", "descrição curta peça 2", "descrição curta peça 3"],
  "moves": [
...
Linhas/colunas = índice 1 a 8 (1=topo/esquerda). Priorize completar linhas."""
        ...
        Thread {
            try {
                ...
                handler.post { showResult("❌ Erro: ${e.message?.take(60)}", emptyList(), false) }
            }
        }.start()
    }
    ...
    private fun showResult(header: String, lines: List<String>, loading: Boolean) {
        ...
        // Sombra via background
        val bg = GradientDrawable().apply {
            setColor(Color.WHITE)
            cornerRadius = 48f
            setStroke(2, 0xFFE2E8F0.toInt())
        }
            ...
            lines.forEach { line ->
                layout.addView(TextView(this).apply {
                    ...
                    setTextColor(when {
                        ...
                        line.startsWith("   Linha") -> 0xFF374151.toInt()
    ...
    private fun buildNotification(): Notification {
        return NotificationCompat.Builder(this, "coach")
            ...
            .setContentText("Toque no 🎯 flutuante para analisar")
